...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    if (is_keyboard_left()) {
        ...
        set_rgb_ledmap(0, 29, rgb_matrix_get_val(), get_highest_layer(layer_state | default_layer_state));
    } else {
        // set RIGHT per-key leds by ledmap
        set_rgb_ledmap(30, 59, rgb_matrix_get_val(), get_highest_layer(layer_state | default_layer_state));
